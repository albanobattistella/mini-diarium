...
The `package-lock.json` generated in step 2 above must also be available in the Flatpak sandbox — `npm ci --offline` requires it. It is declared as an additional file source in the manifest:
...
Like the generated source lists, `package-lock.json` is gitignored and regenerated per release. It is included in the Flathub PR alongside the other generated files.
